...
export function biasCheck(jd:string, bias:Bias | undefined):string {
  ...
  if (bias != undefined) {
    bias.masculine_words.forEach(masculine => {
      if (jd.includes(masculine)) jd = jd.replaceAll(masculine, `<span style="background-color:#F9D689">${masculine}</span>`)
    })

    bias.feminine_words.forEach(feminine => {
      if (jd.includes(feminine)) jd = jd.replaceAll(feminine, `<span style="background-color:#F9D689">${feminine}</span>`)
    })

    bias.warning_words.forEach(warning => {
      if (jd.includes(warning)) jd = jd.replaceAll(warning, `<span style="background-color:#FB6D48">${warning}</span>`)
    })

    bias.banned_words.forEach(banned => {
      if (jd.includes(banned)) jd = jd.replaceAll(banned, `<span style="background-color:#A91D3A">${banned}</span>`)
    })
  }
